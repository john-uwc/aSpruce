...
/**
 * Created by steven on 2016/7/26.
 * 设置 RecyclerView 的间距
 */
public class SpaceItemDecoration extends RecyclerView.ItemDecoration {
    private @Px int mRightSpace;    // 右侧分割线宽度
    private @Px int mBottomSpace;   // 底部分割线高度

    public SpaceItemDecoration(@Px int space) {
        this(space, space);
    }

    public SpaceItemDecoration(@Px int rightSpace, @Px int bottomSpace) {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        int rightSpace  = isLastCol(itemPosition, rowCount, colCount)? 0: mRightSpace;      // 最后一列
        int bottomSpace = isLastRaw(itemPosition, rowCount, colCount)? 0: mBottomSpace;     // 最后一行
